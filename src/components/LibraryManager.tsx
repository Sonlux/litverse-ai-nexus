...
const LibraryManager = ({ onLibrarySelect }: LibraryManagerProps) => {
  ...
  const fetchLibraries = async () => {
    try {
      ...
      const librariesWithCount = data?.map(lib => ({
        ...lib,
        _count: { pdfs: lib.pdfs?.length || 0 }
      })) || [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {libraries.map((library) => (
          <Card key={library.id} className="bg-gray-900 border-gray-700 hover:border-red-500 transition-colors">
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start">
                <CardTitle className="text-white text-lg">{library.name}</CardTitle>
      ...
      {libraries.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-400 mb-2">No Libraries Yet</h3>
          <p className="text-gray-500 mb-4">Create your first library to get started with organizing your PDFs</p>
        </div>
      )}
    </div>
  );
